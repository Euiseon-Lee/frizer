...
- 히스토리 행 전체를 기존 음식명 링크의 클릭 영역으로 확장했다. 태그·변경 내용·시각·빈 공간을 눌러도 같은 목적지로 이동한다. 별도 JavaScript 없이 기본 링크 동작과 키보드 접근을 유지한다.
- 직전 호버 안내는 사용자 요청으로 제거했다. 개별 구매 항목/현재 음식의 구매 목록/연결 대상이 없을 때 전체 목록으로 이동하는 기존 목적지는 유지한다.