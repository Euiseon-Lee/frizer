...
Render Free는 요청이 없는 동안 절전 상태가 될 수 있어 첫 접속이 느릴 수 있다. 잠시 기다린 후 다시 확인한다. 재시작·새 배포 시 메모리에 저장된 로그인 세션은 사라질 수 있으므로 다시 로그인한다. 음식 데이터는 Neon에 남는다. 무료 서비스의 제한은 변경될 수 있으므로 [Render Free 안내](https://render.com/docs/free)를 참고한다.
...
| 프로필 | `prod,render` |
| DB | Neon PostgreSQL, 현재 Flyway V15 |
| 정상 확인 경로 | `/health` |
...
음식이 없으면 마지막 조회는 결과가 없을 수 있다. 정상이다. 앱은 시작할 때 새 Flyway migration을 적용한다. 이미 적용한 V1~V15 파일을 고치지 않고 다음 변경은 V16 이후 새 파일로 추가한다.
...
3. 5절의 환경변수와 `/health`를 설정한다. 새로운 빈 DB일 때만 최초 앱 계정 값을 지정한다. 환경변수 값 칸에 불필요한 바깥 따옴표를 붙이지 않는다.
...
- 검증 기준: Java 515개·JS 7개 파일과 JAR 빌드 통과, 두 운영 계정 HTTPS 로그인·주요 화면·로그아웃 확인, 여백 24조합 비교. 실기기 LTE/5G와 백업 복원은 미검증이다.
- 수량 분리·오등록 삭제·통계와 관리자 전체 사용자 조회는 후속 범위다. 먼저 [현재 구현 상태](STEP3_REPORT.md), [회귀 검증 범위](STEP3_TEST_MATRIX.md), [UI 기준](UI_DESIGN.md)을 읽고 다음 작업 범위를 정한다.
