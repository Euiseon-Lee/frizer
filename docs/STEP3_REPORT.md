# FR!ZER STEP 3 설계 및 구현 범위

## 현재 구현 상태

2026-09-16 기준. 음식 그룹·개별 구매, 신규/기존 음식 추가, 수정, 전체/개별 이동, 부분 소비·폐기·제한적 취소, 종료 목록과 수량 변경 이력을 구현했다.

- 실행·최신 검증: [개발 안내](README.md)
- 수량과 종료: [수량 설계](ITEM_QUANTITY_DESIGN.md)
- 개별 이동: [이동 설계](ITEM_MOVE_DESIGN.md)
- 화면·태그·버튼·날짜 표기: [UI 규격](UI_DESIGN.md)
- 회귀 시나리오: [테스트 범위](STEP3_TEST_MATRIX.md)
- 미구현 일괄 등록: [초안](BULK_REGISTRATION_DRAFT.md)

아래는 전체 STEP 3 설계다. 구현 여부는 위 상태와 기능별 기준 문서를 우선한다. 통계, 수량 일부 분리, 오등록 물리 삭제, 일괄 등록은 후속 범위다. 작업별 시행착오와 과거 테스트 실행 수치는 Git 이력에서 확인한다.
...
현재 검증 결과와 실행법은 [개발 안내](README.md), 회귀 시나리오와 한계는 [테스트 범위](STEP3_TEST_MATRIX.md)를 참조한다.

## 남은 범위

- 수량 일부 분리, 종료 항목 개별 이동, 오등록 물리 삭제
- 일괄 등록·추가 구매, 업로드 요청/행 중복 방지
- 최초 구매량 정정 및 통계·추천 정책, 대표 단위 매핑
- 검색·정렬 확장, 실제 모바일 기기·IME·부하 검증
- PWA·인증·배포는 별도 범위

이미 구현된 소비·폐기·취소·종료·구조화 수량 이력은 후속 목록에서 제외한다. 기능 변경 시 기준 문서를 갱신하고 일회성 실행 기록은 build/reports에 둔다.
