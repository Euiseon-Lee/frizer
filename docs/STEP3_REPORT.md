...
상태: **음식 그룹·병합 및 기존 음식 추가 등록 구현 / 이동·분리·소비·폐기 등 후속 기능 미구현.** 사용자 확정 정책과 구현 제안을 구분해 기록한다.
...
- 현재 수량은 숫자와 단위 사이 공백 하나를 표시한다(예: `5 pieces`, `2 모`). 구조화 수량에서 화면용 문자열만 만들며 저장값·과거 이력·비구조화 원문은 변경하지 않는다.
...
| 엑셀 등록·추가 구매 검증 | 미실행 |
| 전체 Java 테스트 및 JavaScript 회귀 검증 | 2026-09-14 최종 Java 192개, 실패·오류·건너뜀 0 / 이미지 선택기·합치기 JS 검사 통과 |
