...
- 보관 중 전체 선택은 종료 기록까지 함께 병합하고 원래 음식이 사라진다. 히스토리는 전체·일부 관계없이 항목별 MOVE 행으로 남는다(2026-09-17 확정: 보관 중·종료 항목 각각 1건, MERGE 1건 기록과 merge 쓰기 경로 제거, 과거 MERGE 영수증은 표시 유지). 일부 선택은 남는 항목·종료 기록을 원래 음식에 보존한다. 세부 계약은 [이동 설계](ITEM_MOVE_DESIGN.md)를 따른다.
...
| 조리 후 재보관 (단위·음식 변경) | 미구현, 2차 범위 | 그때까지 재료 소비 + ‘직접 조리’ 등록 조합 사용 |
| 오등록 물리 삭제 | 미구현, 항목과 관련 이력 삭제 원칙 | 운영 전 포함 여부 결정 필요 |
| 없어졌어 | 미구현, 소비·폐기와 구분하는 방향 | 후속 |
| 통계 | 미구현 | 운영 이후 (종료 항목 이동은 2026-09-17 다건 이동에 포함) |
...
최근 검증: 전체 Java 528개·JavaScript 48개 통과(2026-09-17, 병합·분리 확정 반영). 사용자가 8080에서 최종 UI를 확인했다. 실제 모바일 기기 점검과 운영 환경 선택은 남아 있다.
...
### 병합·분리 사용자 확정 반영 — 2026-09-17

수량 분리(V17)·다건 선택 이동(V18) 위에서 같은 날 사용자 결정을 반영했다. 세부 계약은 [이동 설계](ITEM_MOVE_DESIGN.md)·[수량 설계](ITEM_QUANTITY_DESIGN.md)·[UI 규격](UI_DESIGN.md)을 따른다.

- **히스토리 층위**: 전체 병합도 merge 위임 없이 항목별 MOVE 영수증으로 기록(보관 중·종료 각각 1건). `FoodMasterService.merge/preview`와 merge 영수증 쓰기 DAO를 제거했고, 과거 `food_merge_receipt`는 표시·리다이렉트를 유지한다. 히스토리 태그 라벨은 MOVE·과거 MERGE 모두 ‘병합’(색상은 기존 이동 태그) — 저장 이벤트 종류는 그대로다.
- **분리 이력 표시**: 히스토리·홈 목록은 원본 SPLIT_OUT 1건으로 합쳐 표시(“새 항목: +수량” 포함), SPLIT_IN은 목록에서 제외. 저장과 항목별 수량 이력은 2건 유지.
- **표현**: ‘다른 음식으로 이동’→‘다른 음식으로 병합’ 계열 전면 교체, 분리 ‘나누자’/‘응, 나누자’, 등록 ‘등록하자!’, 수정 ‘수정하자!’·‘정보 수정 / 정보가 달라졌어?’, 병합 완료 알림은 2줄로 표시.
- **레이아웃**: 소비/폐기/분리 화면을 병합 화면 기준으로 통일(헤딩 블록+쪼코, 흰색 form-section 카드, quantity-message 13px·양옆 8px, 전폭 merge-submit). 분리 입력은 등록 양식 재사용(수량/단위 2열·단위 disabled, 보관 위치 버튼 3종 위치 색상, freezer-fields 냉동실 선택 시 표시 — food-split.js). 분리의 개봉일 입력은 제거하고 자식이 원본 개봉일을 상속한다.
- **버그 수리**: 홈 배너 버튼화 때 남은 `.add-banner span` CSS 2건을 anchor 선택자로 교정. 68ab6fa가 남긴 bulk-registration JS 테스트 무한 대기(삭제된 다중 파일 분기 케이스)를 정리.
- 검증: 전체 Java 528개·JavaScript 48개 통과.
