...
2026-09-16 개인 로그인·Render 배포 준비 후 전체 재검증: Java 496개, 실패·오류·건너뜀 0, JAR 빌드 통과. SecurityIntegrationTest 18개에서 비로그인 조회 차단·실제 비밀번호 로그인/로그아웃·CSRF 누락/위조 차단·폼 토큰·멀티파트 헤더·단일 오류 템플릿의 403/404/500 분기를 검증했다. JavaScript 7개 파일도 통과했으며 보안 업로드와 로그인 만료 복귀 2개를 추가했다. 실제 Neon PostgreSQL 18.6 임시 브랜치에서 V1~V14와 TLS 연결·비웹 JAR 기동 통과. Render 웹 배포와 휴대폰 확인은 별도다.
...
## 2026-09-16 작업 마무리 검증

- 전체 Java 테스트 473개: 실패 0, 오류 0, 건너뜀 0. `test bootJar` 통과.
- 전체 JavaScript 테스트 38개: 실패 0, 건너뜀 0.
- 일괄 등록: 신규/추가 시트, 합계 500개 제한, 파일·행 검증, 기존 음식 연결·버전 충돌, 원본 서식 보존, 중복·동시 제출, 전체 롤백, 세션·만료를 검증한다.
- 자동 보정 안내는 냉동 구분 미선택, 배달 잔반과 보관 위치 미선택, 배달 잔반과 냉동 구분 선택을 분리해 메시지 및 결과를 검증했다.
- JS는 파일 변경 시 이전 요청 무효화, 등록 비활성화, 미리보기 부분 갱신·02 포커스, 세션 오류·통신 실패 후 재시도를 검증한다.
- 수량 소수 둘째 자리, CREATE 수량 이력, 수정 후 필터를 유지한 개별 구매 목록 복귀는 회귀 검증에 포함한다.
- 사용자가 8080에서 안내 간격·카드 상세 정보·확인 필요 필터의 빈 결과 UI를 최종 확인했다. 실기기·대규모 데이터 부하·외부 운영 배포는 이번 검증에 포함하지 않는다.
- 최종 정리에서 제거한 중복/미사용 CSS는 화면 동작을 바꾸지 않는다. 실행 리소스와 JAR는 정리본으로 다시 생성한다.

자동 미리보기 전환 후 검증: 일괄 등록 JS 7개. 파일 선택 자동 요청, 미선택·복수 파일 차단, 결과 갱신·02 포커스, 세션·통신 실패 후 동일 파일 재선택, 파일 교체·선택 해제 시 이전 응답 무시를 검증한다.

중복 확인 UI 검증: JS 체크·해제에 따른 활성화, 파일 교체 후 이전 체크로 재활성화 방지, 일반 미리보기 활성화, 서버의 중복 확인 체크박스·비활성 버튼 렌더링과 상단 요약 제거를 추가 검증한다.

필수 누락 집계 검증: 신규 등록의 음식명·수량·단위·보관 위치 누락 집계, 0·음수·소수 셋째 자리·수량 상한·비숫자 입력과 누락의 분리, 추가 등록 미선택 시 음식명 오류 생략, 보관 위치 선택 오류와 누락 분리를 검증한다.
# 2026-09-16 후속 검증: 일괄 등록 문구

- 실행: `test --tests '*BulkRegistrationIntegrationTest' bootJar --offline`, 44개 통과(실패·오류·건너뜀 0).
- 추가: 잘못된 보관 위치·출처·냉동 구분의 종속 오류/자동 안내 억제, 미입력 보관 위치의 냉동 조건 오판 방지, 수식 수량 단일 오류, 독립 조건 오류와 마지막 누락 집계, 이전 미리보기의 재업로드 안내와 무반영.
- 기존 정상 등록·중복 방지·전체 원자성·세션·템플릿 렌더링 회귀를 함께 통과했다.
- 서버 Compose 병합 설정 검증 통과. 실제 Render 웹 배포·HTTPS·휴대폰 접속은 미검증이다.
- 전체 테스트 수는 이 부분 실행 결과로 갱신하지 않는다.
