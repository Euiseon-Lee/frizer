# FR!ZER 구현 범위 테스트 목록 및 결과

## V15 사용자 격리 검증 범위 (운영 반영 전)

`UserIsolationIntegrationTest`는 실제 PostgreSQL과 보안 필터로 검증한다. 본인/테스트 두 계정은 USER이며 운영 계정·운영 데이터는 테스트에서 만들거나 변경하지 않는다.

- 두 계정의 실제 폼 로그인, 로그인 ID 변경 후 불변 소유권, 비활성 계정의 기존 세션/재로그인 차단. 로그인 시 세션 ID 교체, 로그아웃 후 폐기, 비밀번호·등급 변경 및 비활성→재활성 후 이전 세션 차단, 두 세션 동시 사용과 같은 브라우저 계정 전환 후 엑셀 미리보기 격리.
- 홈/목록/상세/음식 선택지/이력/수량 이력/종료 재고의 소유자 분리, 타 사용자 ID 직접 요청 404.
- 위조 수정·음식 삭제 Mapper·추가 등록·병합·개별 이동·소비/취소 차단과 원본 유지.
- 같은 등록 UUID의 사용자별 독립 처리, 타 사용자 영수증 조회 차단.
- 세션 토큰까지 알고 있어도 타 계정 엑셀 commit 거부, 동일 파일 fingerprint 사용자별 독립 처리, 다른 사용자 음식 번호의 추가 등록 거부.
- 소유자 없는 INSERT 및 사용자 혼합 FK 거부, 미인증 서비스 접근 차단, 같은 트랜잭션 내 MyBatis 캐시의 사용자 분리.
- V14의 9개 업무 테이블 데이터 보존·본인 사용자 귀속, 수량/수정 시각/버전 유지, 잠긴 전환 계정.
- 운영자 계정 추가 명령의 USER 기본값, 비밀번호 해시, 기존 계정 덮어쓰기 거부. 공개 회원가입 없음.

최종 검증: Java 515개(실패·오류·건너뜀 0), JavaScript 7개 파일, bootJar 통과. 10자 비밀번호 정책과 조건별 사용자 안내 문구 검증을 포함한다.
...
정책 기준: [앱 개념 문서](APP_CONCEPTS.md). 소비·폐기·취소 및 개별 이동은 이후 구현했고 FoodQuantityIntegrationTest와 FoodItemMoveIntegrationTest 등의 코드가 실행 가능한 회귀 기준이다. 부분 분리·물리 삭제·엑셀은 미구현이다.
...
정확한 메서드·매개변수 실행 목록은 [Java 실행 목록](TEST_MATRIX.md)과 테스트 소스에서 확인한다.
...
후속 검증에는 WARNING 0/1/3/4/100개, 날짜 미입력/과거/오늘/미래, 동일 수량 변경 이력 제외를 포함한다. 최종 날짜 카드·버튼·아이콘 수정까지 포함한 실행이며 브라우저 확인은 각 변경 당시 기록과 구분한다. [최종 상태](UI_DESIGN.md)와 [전체 실행 목록](TEST_MATRIX.md) 참고.
...
이 경로는 이후 일부 테스트만 실행하면 내용이 바뀐다. 이번 최종 실행 목록은 [별도 목록](TEST_MATRIX.md)에 보존했다.
...
- 미구현 기능은 구현 후 같은 방식으로 목록과 검증을 추가한다.
...
## 2026-09-17 이미지 로딩 개선 검증

- 원본 PNG 77개와 기존 승인 해시는 보존했다. WebP 사본 77개의 크기·비율과 축소 원본 대비 알파 채널 일치를 확인했다. 합계 250,838,887 → 4,249,030바이트(98.3% 감소). 원본/사본 4종 비교를 확인했으며 임시 비교 화면·검증 스크립트는 작업 정리 시 제거했다.
- JavaScript 전체 8개 파일, Node 테스트 결과 48개 통과. 준비 완료 후 표시, 캐시된 이미지, 고정 로그인 이미지, decode 미지원 대체 동작, 이전 요청 결과 무시, 실패 시 제한적 재시도를 포함한다.
- 배포 전 전체 Java 516개 통과(실패·오류·건너뜀 0), bootJar 통과. SecurityIntegrationTest 19개에 로그인 전 WebP 조회, WebP Content-Type, 버전 이미지 1년 public/immutable 캐시, 로그인 no-store 검증을 포함한다.
- 실제 Chrome의 320/390px·DPR 3 테스트 화면에서 이미지 응답을 지연시켜 로딩 중 숨김, 디코딩 후 표시, 이미지 전후 동일한 영역 크기, 가로 넘침 없음, JS 오류 없음, WebP 요청을 확인했다. 운영 실기기 속도 측정과는 구분한다.
...
2026-09-17 원본 프로젝트 C:\dev\frizer 최종 검증: Java 515개, 실패·오류·건너뜀 0, JavaScript 7개 파일, bootJar 및 diff 공백 검사 통과. V15 전환·두 계정 격리·세션 검증 포함. 히스토리 빈 화면의 하단 쪼코 이미지에만 버튼 위 20px 간격을 추가했다. 배포 전 검증 기록이며, 이후 운영 배포 결과는 OPERATIONS.md를 따른다.

## 2026-09-17 오등록 물리 삭제 검증

- 전체 Java 540개 통과(실패·오류·건너뜀 0), bootJar 통과. 스모크 테스트의 최신 migration 고정값을 V19로 갱신했다.
- FoodDeleteIntegrationTest 12개 통과(실패·오류·건너뜀 0): 취소 이력 포함 항목의 이력 전체 삭제와 형제 항목·요청 영수증 보존, 전체 선택의 빈 그룹 유지와 ‘그룹도 함께 삭제’ 옵션, 일부 선택의 그룹 삭제 거부, 같은 토큰 재실행의 완료 결과 재사용과 다른 내용 거부, 음식·항목 버전 어긋남 거부와 무변경 확인, 삭제 영수증 실패 시 전체 롤백, 삭제된 등록 요청 재전송의 재생성 차단, 종료 항목 삭제, 타 음식 항목·빈 선택 거부, 확인 화면 렌더링·POST 리다이렉트·무선택 진입 복귀, 동시 같은 토큰의 1회 실행.
- UserIsolationIntegrationTest의 위조 작업 차단에 타 사용자 음식의 삭제 미리보기·실행 차단을 추가했다.
- move-select.test.cjs 추가: 병합·삭제 버튼의 선택 연동, BFCache 복귀 재동기화, 단일 버튼·폼 없음 가드. 이번 작업 환경에는 Node가 없어 실행하지 못했다. 다음 검증 시 `node --test src/test/js/*.test.cjs`로 포함한다.
- 같은 날 UI 조정(분류 필드 01 이동→출처 아래 재이동, 상세 정보 재배치·출처 메모 분리, 수정 화면 공유 필드 강조, 냉동 영역 간격, 출처 라벨, 분리·삭제 화면 문구/여백, 버튼·복귀 링크 간격 20px 통일, 체크박스 19px·2색·간격 8px 통일, 추가 등록 전폭, 제목 자동 축소의 숨김 후 일괄 표시)도 전체 실행에 포함해 통과했다. 상세의 출처 마크업 검사는 새 구조(`<dd>-</dd>`)로 갱신했다.
- 히스토리 병합 표시 MERGE 통일(신형·구형 동일 양식, 화살표 강조 전 태그 적용, 병합/이름 변경 안내 문구 분기)을 반영해 병합·이동·재고 테스트의 기대값을 갱신했고, 최종 전체 Java 540개(실패·오류·건너뜀 0)·bootJar 통과를 재확인했다(2026-09-17).
...
- 후속 빈 카드 여백: 템플릿 전체 검색 결과 사용처는 히스토리·재고 목록·개별 구매 3곳이다. 실제 CSS/이미지 기반 브라우저 비교 화면에서 320/390px 폭 × 3개 사용처 × 4개 렌더 모드, 총 24조합의 수정 전후 치수를 비교했다. 히스토리 portrait 두 경우만 하단 padding과 카드 높이가 28px 증가했고 내부 요소 위치는 동일했다. 나머지 22조합은 동일했다. 실기기 전체 페이지 검증과는 구분한다. 임시 비교 화면은 build/reports에 두고 커밋하지 않는다.
