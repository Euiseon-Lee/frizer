...
public interface FoodQuantityDao {
    ...
    @Select("SELECT h.history_id,h.action_type,h.before_quantity_amount,h.after_quantity_amount,h.before_quantity_unit,h.quantity_unit,h.quantity_text,h.created_at FROM food_history h WHERE h.user_id=#{_userId,jdbcType=BIGINT} AND h.food_id=#{id} AND (h.action_type IN ('CREATE','SPLIT_IN') OR (h.action_type IN ('CONSUME','DISCARD','CANCEL','SPLIT_OUT') AND h.operation_id IS NOT NULL) OR (h.action_type='UPDATE' AND (h.before_quantity_amount IS DISTINCT FROM h.after_quantity_amount OR h.before_quantity_unit IS DISTINCT FROM h.quantity_unit))) ORDER BY h.history_id DESC")
    ...
    record RegistrationQuantity(long foodId, String registrationQuantity) {}
    @Select("SELECT i.food_id, (SELECT h.quantity_text FROM food_history h WHERE h.user_id=#{_userId,jdbcType=BIGINT} AND h.food_id=i.food_id AND h.action_type IN ('CREATE','SPLIT_IN') ORDER BY h.history_id LIMIT 1) AS registration_quantity FROM food_item i WHERE i.user_id=#{_userId,jdbcType=BIGINT} AND i.status='DEPLETED'")
    java.util.List<RegistrationQuantity> endedRegistrationQuantities();
    @Select("SELECT i.food_id, (SELECT h.quantity_text FROM food_history h WHERE h.user_id=#{_userId,jdbcType=BIGINT} AND h.food_id=i.food_id AND h.action_type IN ('CREATE','SPLIT_IN') ORDER BY h.history_id LIMIT 1) AS registration_quantity, (SELECT h.created_at FROM food_history h WHERE h.user_id=#{_userId,jdbcType=BIGINT} AND h.food_id=i.food_id AND h.action_type IN ('CONSUME','DISCARD') AND h.after_quantity_amount=0 AND h.operation_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM food_history c WHERE c.reversal_of_history_id=h.history_id) ORDER BY h.history_id DESC LIMIT 1) AS ended_at FROM food_item i WHERE i.user_id=#{_userId,jdbcType=BIGINT} AND i.master_id=#{masterId} AND i.status='DEPLETED'")
