...
public interface FoodQuantityDao {
    ...
    @Select("SELECT i.food_id, (SELECT h.quantity_text FROM food_history h WHERE h.food_id=i.food_id AND h.action_type='CREATE' ORDER BY h.history_id LIMIT 1) AS registration_quantity, (SELECT h.created_at FROM food_history h WHERE h.food_id=i.food_id AND h.action_type IN ('CONSUME','DISCARD') AND h.after_quantity_amount=0 AND h.operation_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM food_history c WHERE c.reversal_of_history_id=h.history_id) ORDER BY h.history_id DESC LIMIT 1) AS ended_at FROM food_item i WHERE i.master_id=#{masterId} AND i.status='DEPLETED'")
