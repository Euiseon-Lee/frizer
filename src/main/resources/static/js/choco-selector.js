...
const groups={
 ...
 NEUTRAL:['proud-sit','puppy-tilt','puppy-ready','look-aside','calm-closeup','red-collar-puppy','puppy-paw-reach','puppy-look-down'],
 HAPPY:['come-running','extra-sunny-sit','extra-tongue-step','happy-lounge','happy-sit','puppy-front-paws','happy-run-front','belly-up-play']
};
const wide=['side-rest','belly-up-lounge'];
...
for(const [emotionGroup,keys] of Object.entries(groups))for(const key of keys){
 assets[key]={key,emotionGroup,poseGroup:poses[key],
  allowedRoles:Object.keys(policies).filter(role=>scopes[policies[role].scope].includes(key)),
  renderMode:wide.includes(key)?'wide':key==='sniff'?'left-edge':['happy-closeup','empty-curious','rest','calm-closeup','back-view-harness'].includes(key)?'portrait':'contain'};
}
